...
static int test_card(const char* path) {
    ...
    if (!host || IS_ERR(host)) { printk("bad host\n"); return -ENODEV; }
//    pdev = get_pdev(host);
//    if (!pdev || IS_ERR(pdev)) return -ENODEV;
//    printk("removing mmc_host");
//    host_off(host);
    printk("claiming mmc_host");
    mmc_claim_host(host);

    /* gpio88mode2 */
    printk("pulling down gpio 88\n");
    config_gpio_table(emmc_wp_reset2_table, ARRAY_SIZE(emmc_wp_reset2_table));
    mdelay(100);
    printk("returning gpio 88 to normal\n");
    config_gpio_table(emmc_wp_normal2_table, ARRAY_SIZE(emmc_wp_normal2_table));

    /* gpio88mode3 */
    printk("pulling down gpio 88\n");
    config_gpio_table(emmc_wp_reset3_table, ARRAY_SIZE(emmc_wp_reset3_table));
    mdelay(1000);
    printk("returning gpio 88 to normal\n");
    config_gpio_table(emmc_wp_normal3_table, ARRAY_SIZE(emmc_wp_normal3_table));
    printk("done.\n");

//    printk("restoring in 5 seconds\n");
//    msleep(5);
//    rereg();
    printk("releasing mmc_host");

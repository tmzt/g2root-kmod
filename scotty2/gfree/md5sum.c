...
/* Modified by Guhl to remove additional dependencies, 2012.  */
...
static int md5_check(const char *checkfile_name)
{
  ...
  do {
    ...
    if (split_3(line, line_length, &md5num, &binary, &filename)
        || !hex_digits(md5num)) {
      if (warn) {
    	fprintf(stderr, "%s: %lu: improperly formatted MD5 checksum line\n",
  ...
  if (ferror(checkfile_stream)) {
	fprintf(stderr, "%s: read error", checkfile_name); /* */
    return FALSE;
  }
  ...
  if (n_properly_formated_lines == 0) {
	...
	fprintf(stderr, "%s: no properly formatted MD5 checksum lines found\n",
             checkfile_name);
    return FALSE;
  } else {
    if (!status_only) {
      ...
      if (n_open_or_read_failures > 0) {
        fprintf(stderr, "WARNING: %d of %d listed files could not be read\n",
